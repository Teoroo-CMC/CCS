...
def ew(atoms,q):
    ...
    struct= Structure(lattice,atoms.get_chemical_symbols(), coords, site_properties={"charge" : atoms.charges})
...
class CCS(Calculator):
    ...
    def calculate(self, atoms=None, properties=['energy'],system_changes=all_changes):
        ...
        if 'numbers' in system_changes:
            self.nl = NeighborList([self.rc / 2] * natoms, bothways=True, self_interaction=False)
        ...
        for at in range(natoms):
            ...
            for i , offset in zip(indices,offsets):
                ...
                f = self.pair[elem1+elem2].eval_force(d)*(d_vector/d) # check this
