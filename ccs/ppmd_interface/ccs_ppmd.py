...
def newtonic(A, constants):
    ...
    NK = ParticleLoop(
        kernel=N_kernel,
        dat_dict={'F': A.force(access.READ), 'P': A.pos(access.INC), 'F_old': A.force_old(access.INC), 'P_old': A.pos_old(access.INC)})
    ...
    def opt(self, A, steps=10000, fmax=0.05):
        ...
        for it in range(steps):
            ...
            if not uphill:
                self.F4.execute()
                self.F5.execute()
            MaxF = np.max(np.linalg.norm(
                A.force[0:A.npart], axis=1))*internal_to_ev()
            if A.domain.comm.rank == 0 and it % 1 == 0:
                print("{: 8d} | E: {: 12.8f} F: {: 12.8f}  a: {: 12.8f} dt: {: 12.8g}  Fv: {: 12.8f} ".format(
                    it, Eold, MaxF, A.a[0], A.dt[0], A.Fv[0]))
            if (MaxF < fmax):
                t2 = time()
                print(
                    "------------------------------- OPTIMIZATION COMPLETE -------------------------------")
                print("    Total time for opimization: ", t2-t1, " s, Max force:",
                      np.max(np.linalg.norm(A.force.view*internal_to_ev(), axis=1)), "ev/Å")
                break
...
class CCS():
    def __init__(self, A, atoms, q=None, CCS_params='CCS_params.json'):
        ...
        if np.linalg.norm(charges) > 0.0001:
            try:
                ...
                es = coulomb.ewald.EwaldOrthoganal(A.domain, real_cutoff=10.)
            self.es = es
        else:
            self.es = None
            print("SWITHING OF +Q.")
    ...
    def eval(self, A, pos=None):
        ...
        if self.es:
            ...
            E += Ee
        return (E+self.onebody)*internal_to_ev(), -A.force.view.flatten()*internal_to_ev()
...
def optimize(A, atoms, CCS, Type='LBGS', Fmax=0.05):
    ...
    if (Type == 'LBGS'):
        options = {'disp': 1, 'iprint': 101,
                   'maxiter': 1000, 'gtol': Fmax, 'ftol': 1E-12}
        method = 'L-BFGS-B'

    if (Type == 'CG'):
        options = {'disp': 1, 'iprint': 101,
                   'maxiter': 1000, 'gtol': Fmax, 'norm': np.inf}
        method = 'cg'

    t1 = time()
    min(_fun, A.pos[0:A.npart].flatten(), args=(CCS, A),
        method=method,
        jac=True,
        options=options)
    t2 = time()
