...
class Objective:
    ...
    def __init__(
        ...
        self.ref = np.hstack(
            (self.energy_ref, self.force_ref)
    ...
    def get_m(self):
        ...
        fvv_x = np.hstack([*tmp])
        fvv_x = np.hstack(
            (fvv_x, np.zeros((self.l_twb[ii].Nconfs_forces, np.shape(self.sto)[1])))
        )
        ...
        fvv_y = np.hstack([*tmp])
        fvv_y = np.hstack(
            (fvv_y, np.zeros((self.l_twb[ii].Nconfs_forces, np.shape(self.sto)[1])))
        )
        ...
        fvv_z = np.hstack([*tmp])
        fvv_z = np.hstack(
            (fvv_z, np.zeros((self.l_twb[ii].Nconfs_forces, np.shape(self.sto)[1])))
        )
