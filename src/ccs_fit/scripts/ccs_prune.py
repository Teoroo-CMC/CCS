...
def main():
    ...
    parser.add_argument("-dc", "--CCS_DB", type=str, metavar="",
                        default='CCS_pruned.db',  help="Name of data-base to store results")
    parser.add_argument("-p", "--CCS_params", type=str, metavar="",
                        default='CCS_params_pre_prune.json',  help="CCS_params_pre_prune.json file")
    ...
    ccs_fetch(
        mode="DFTB", DFT_DB=vars(args)["DFT_DB"], R_c=vars(args)["R_c"], Ns=vars(args)["Ns"], DFTB_DB=vars(args)["CCS_DB"], charge_dict=vars(args)["charge_dict"], include_forces=vars(args)["include_forces"], verbose=vars(args)["verbose"])
