...
class fit_task:
    def __init__(self,pairs=None,crystal=None,Tm=0.0,r_cut=0.0,V_ij=None,charge_dict=None,damping=0.5,CCS_res=0.1):
        self.pairs=pairs
    ...
    def init_training(self,min_scale=0.0,max_scale=1E12,vol_scan=None,scan=True):
        calc = G2B(G2B_params=self.G2B_input,charge=True,q=self.charge_dict,charge_scaling=True)
        ...
        if scan:
            ...
            for scale in np.arange(0.8, 1.30,0.01):
                ...
                if nrg-Emax < 0:
                    counter+=1
                    print("Found structure with scale : ",scale," and energy", nrg)
                vols.append((scale**3)*self.train_cell.get_volume()/len(self.train_cell))
                Es.append(nrg)
    ...
    def scramble(self,size=10,DB=None,damping=None):
        ...
        while counter  < size:
            scale=np.random.random()*(self.max_scale - self.min_scale)+self.min_scale
            new_struct = self.train_cell.copy()
            new_struct.set_cell(self.train_cell.cell * scale, scale_atoms=True)
            ...
            if nrg-self.E0-self.Espan < 0:
                ...
                print("Added strucutre No: ",counter," with E: ",nrg,"  Scale: ",scale)
    ...
    def compare_q(self,UNC=False):
        ...
        for sp in self.charge_dict:
            print(sp," :",sq*self.charge_dict[sp] ," vs target:",self.charge_dict[sp])
        return np.abs(sq-1.)*100.
    ...
    def plot_summary(self):
        ...
        for pair in self.pairs:
            ...
            plt.plot(r,np.array(e),'--',color='red',label="Fitted potential "+pair)
            plt.plot(r,np.array(e_UNC),'--',color='blue',label="Fitted unconst. potential "+pair)
        ...
        plt.plot( [min(err_F[:,0]),max(err_F[:,0])],[min(err_F[:,0]),max(err_F[:,0])],'--',color='black')
        plt.scatter(err_F[:,0],err_F[:,1],facecolors='none', edgecolors='red',alpha=0.8 )
        plt.show()
        ...
        #ax.set_ylim(-2,2)
        ax.hexbin(err_F[:,1],y=1000*(err_F[:,1]-err_F[:,0]), gridsize=20,cmap='Blues')
        plt.show() 
    ...
    def check_sampling(self):
        ...
        ax.set_xlim(self.min_scale**3*self.train_cell.get_volume()/len(self.train_cell)-1,self.max_scale**3*self.train_cell.get_volume()/len(self.train_cell)+1)
        ax.plot([self.min_scale**3*self.train_cell.get_volume()/len(self.train_cell),self.max_scale**3*self.train_cell.get_volume()/len(self.train_cell)],[self.E0+self.Espan,self.E0+self.Espan],"--",color="black")
    ...
    def calculate_overlap_mae(self,UNC=False):
        ...
        for pair in self.pairs:
            ...
            rs=np.arange(CCS_params["Two_body"][pair]["r_min"],CCS_params["Two_body"][pair]["r_cut"],0.001)
    ...
    def calculate_overlap_rmse(self,UNC=False):
        ...
        for pair in self.pairs:
            ...
            I = trapz(abs_de, rs)
            overlap[pair]= (I/(rs[-1]-rs[0]))**0.5 # Integral divided by interval lenght
