...
class fit_task:
    ...
    def compare_q(self, UNC=False):
        ...
        for sp in self.charge_dict:
            print(
                sp, " :", sq * self.charge_dict[sp], " vs target:", self.charge_dict[sp]
            )
    ...
    def plot_summary(self):
        ...
        for pair in self.pairs:
            ...
            plt.plot(
                r, np.array(e), "--", color="red", label="Fitted potential " + pair
            )
            plt.plot(
                r,
                np.array(e_UNC),
        ...
        try:
            ...
            plt.scatter(
                err_F[:, 0], err_F[:, 1], facecolors="none", edgecolors="red", alpha=0.1
            )
            plt.show()
        except:
            pass
        ...
        plt.scatter(
            err_F[:, 0], err_F[:, 1], facecolors="none", edgecolors="red", alpha=0.8
        )
        plt.show()
        ...
        # ax.set_ylim(-2,2)
        ax.hexbin(
            err_F[:, 1], y=1000 * (err_F[:, 1] - err_F[:, 0]), gridsize=20, cmap="Blues"
        )
        plt.show()
